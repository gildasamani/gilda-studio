...
function notificationHtml(data: {
  ...
  const row = (label: string, value: string) =>
    `<tr>
      <td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.05);font-size:11px;letter-spacing:0.12em;text-transform:uppercase;color:rgba(183,123,87,0.55);font-family:Arial,sans-serif;width:140px;vertical-align:top;">${label}</td>
      <td style="padding:10px 0 10px 16px;border-bottom:1px solid rgba(255,255,255,0.05);font-size:13px;color:rgba(216,197,174,0.8);line-height:1.6;font-family:Arial,sans-serif;">${value || "&mdash;"}</td>
    </tr>`;
...
router.post("/contact", limiter, async (req, res) => {
  ...
  if (_honey) {
    req.log.warn({ ip: req.ip }, "Honeypot triggered — bot submission blocked");
  ...
  if (!name?.trim() || !email?.trim() || !projectType?.trim() || !budget?.trim() || !message?.trim()) {
    req.log.warn({ fields: { name: !!name, email: !!email, projectType: !!projectType, budget: !!budget, message: !!message } }, "Contact form missing required fields");
  ...
  if (!emailOk) {
    req.log.warn({ email }, "Contact form invalid email format");
  ...
  if (!apiKey) {
    req.log.error("RESEND_API_KEY environment secret is not set — email sending is disabled");
  ...
  try {
    const [notif, confirm] = await Promise.all([
      resend.emails.send({
        from: FROM,
        to: STUDIO_EMAIL,
        subject: `New enquiry from ${name.trim()} — Gilda Studio`,
    ...
    req.log.info({ notifId: notif.data?.id, confirmId: confirm.data?.id }, "Contact emails sent successfully");
    res.status(200).json({ success: true });
  } catch (err) {
    req.log.error({ err, name, email, projectType, budget }, "Failed to send contact emails via Resend");
